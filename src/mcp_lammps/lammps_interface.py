...
"""
        ...
        # Note: Type consistency validation is deprecated with OpenFF
        # OpenFF Interchange ensures consistency automatically
        if validate_consistency and FORCEFIELD_AVAILABLE:
            try:
                logger.info("Type consistency validation skipped - OpenFF Interchange ensures consistency")
                script += "\n# Type consistency: Ensured by OpenFF Interchange\n"
            except Exception as e:
                logger.warning(f"Type consistency note failed: {e}")
                script += f"\n# Type consistency validation: N/A\n"
