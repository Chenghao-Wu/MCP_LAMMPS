...
def register_organic_tools(server: Any, lammps_server: Any) -> None:
    ...
    @server.tool(
        ...
        description="Convert SMILES to 3D structure with OpenFF force field"
    )
    async def import_from_smiles(
        ...
        """
        Convert SMILES string to 3D structure with OpenFF force field.
        ...
        """
        try:
            # Import structure from SMILES using OpenFF
            ...
            ctx.info(f"Successfully imported molecule from SMILES using OpenFF: {molecule_name}")
    ...
    @server.tool(
        ...
        description="Complete organic liquid simulation setup using OpenFF"
    )
    async def create_organic_simulation(
        ...
        """
        Create a complete organic liquid simulation setup using OpenFF.
        
        Args:
            name: Name of the simulation
            molecules: List of molecule dicts with 'smiles', 'count', 'name'
            target_density: Target density in g/cm³
            force_field: Force field to use (openff)
        ...
        try:
            ...
            # Load metadata from liquid box file
            metadata_file = box_file.with_suffix(".json")
            metadata = {}
            ...
            if metadata_file.exists():
                ...
                try:
                    with open(metadata_file, 'r') as f:
                        metadata = json.load(f)
                    ctx.info(f"Loaded OpenFF system metadata: {metadata.get('total_atoms', 0)} atoms, "
                            f"{metadata.get('total_molecules', 0)} molecules")
                except Exception as e:
                    logger.warning(f"Failed to load metadata from {metadata_file}: {e}")

            # Get the script file generated by OpenFF Interchange
            ...
            ctx.info(f"Created organic liquid simulation using OpenFF '{name}' with ID: {simulation_id}")
    ...
    # REMOVED: setup_amber_forcefield tool - OpenFF handles all force field assignment automatically
    
    @server.tool(
        name="create_mixture_simulation",
        title="Create Mixture Simulation",
        description="Multi-component liquid mixture setup using OpenFF"
    )
    async def create_mixture_simulation(
        ...
        """
        Create a multi-component liquid mixture simulation using OpenFF.
        ...
        try:
            ...
            ctx.info(f"Created mixture simulation using OpenFF '{name}' with {len(components)} components")
