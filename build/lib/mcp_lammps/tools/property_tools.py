...
def register_property_tools(server: Any, lammps_server: Any) -> None:
    ...
    @server.tool(
        name="analyze_phase_behavior",
        title="Analyze Phase Behavior",
        description="Analyze phase transitions and behavior"
    )
    async def analyze_phase_behavior(
        ctx: Context,
        simulation_id: str,
        analysis_type: str = "density_temperature",
        window_size: int = 100
    ) -> Dict[str, Any]:
        """
        Analyze phase behavior and transitions.
        
        Args:
            simulation_id: Simulation ID
            analysis_type: Type of analysis (density_temperature, pressure_volume)
            window_size: Window size for moving averages
            
        Returns:
            Phase behavior analysis results
        """
        try:
            # Get simulation
            sim = lammps_server.simulation_manager.get_simulation(simulation_id)
            ...
            if analysis_type == "density_temperature":
                # Analyze density vs temperature relationship
                if 'Temp' in thermo_df.columns and 'Density' in thermo_df.columns:
                    results = _analyze_density_temperature(thermo_df, window_size)
                else:
                    raise ValueError("Temperature and/or Density data not available")
            
            elif analysis_type == "pressure_volume":
                # Analyze pressure vs volume relationship
                if 'Press' in thermo_df.columns and 'Volume' in thermo_df.columns:
                    results = _analyze_pressure_volume(thermo_df, window_size)
                else:
                    raise ValueError("Pressure and/or Volume data not available")
            
            else:
                raise ValueError(f"Unknown analysis type: {analysis_type}")
            
            # Save results
            sim.add_result("phase_behavior", results)
            
            ctx.info(f"Analyzed phase behavior for simulation: {sim.name}")
            ...
            logger.error(f"Failed to analyze phase behavior: {e}")
            raise
    
    @server.tool(
        name="compute_solvation_properties",
        title="Compute Solvation Properties",
        description="Calculate solvation free energy and related properties"
    )
    async def compute_solvation_properties(
        ctx: Context,
        simulation_id: str,
        solute_selection: str = "resname SOL",
        method: str = "thermodynamic_integration"
    ) -> Dict[str, Any]:
        """
        Compute solvation properties including free energy.
        
        Args:
            simulation_id: Simulation ID
            solute_selection: Selection string for solute molecules
            method: Method for free energy calculation
        ...
        try:
            ...
            if not traj_files:
                raise ValueError("No trajectory file found for analysis")
            
            if MDANALYSIS_AVAILABLE:
                # Use MDAnalysis for advanced solvation analysis
                results.update(_compute_solvation_with_mda(traj_files[0], solute_selection))
            else:
                # Simplified solvation analysis
                results.update(_compute_solvation_simple(sim))
            
            # Save results
            sim.add_result("solvation_properties", results)
            
            ctx.info(f"Computed solvation properties for simulation: {sim.name}")
            
            return {
                "simulation_id": simulation_id,
                "name": sim.name,
                "solvation_properties": results
            }
            
        except Exception as e:
            logger.error(f"Failed to compute solvation properties: {e}")
            raise
...
def _analyze_density_temperature(thermo_df: Any, window_size: int) -> Dict[str, Any]:
    """Analyze density vs temperature relationship."""
    try:
        temp = thermo_df['Temp'].values
        density = thermo_df['Density'].values
        
        # Calculate moving averages
        temp_smooth = _moving_average(temp, window_size)
        density_smooth = _moving_average(density, window_size)
        
        # Find phase transitions (large changes in density slope)
        density_gradient = np.gradient(density_smooth, temp_smooth)
        
        # Detect transitions as peaks in gradient magnitude
        gradient_magnitude = np.abs(density_gradient)
        threshold = np.mean(gradient_magnitude) + 2 * np.std(gradient_magnitude)
        
        transition_indices = np.where(gradient_magnitude > threshold)[0]
        
        transitions = []
        for idx in transition_indices:
            if 0 < idx < len(temp_smooth) - 1:
                transitions.append({
                    "temperature": float(temp_smooth[idx]),
                    "density": float(density_smooth[idx]),
                    "gradient": float(density_gradient[idx])
                })
        ...
        return {
            ...
            "density_temperature_correlation": float(np.corrcoef(temp, density)[0, 1]),
            "analysis_method": "gradient_detection"
        }
        
    except Exception as e:
        logger.error(f"Error analyzing density-temperature: {e}")
        return {"error": str(e)}


def _analyze_pressure_volume(thermo_df: Any, window_size: int) -> Dict[str, Any]:
    """Analyze pressure vs volume relationship."""
    try:
        ...
        # Calculate moving averages
        pressure_smooth = _moving_average(pressure, window_size)
        volume_smooth = _moving_average(volume, window_size)
        
        # Fit PV relationship (could indicate phase behavior)
        # Try different models: linear, power law
        
        # Linear fit
        linear_coeffs = np.polyfit(volume_smooth, pressure_smooth, 1)
        linear_r2 = _calculate_r_squared(volume_smooth, pressure_smooth, linear_coeffs)
        
        # Power law fit: P = a * V^b
        try:
            log_p = np.log(np.abs(pressure_smooth) + 1e-10)
            log_v = np.log(np.abs(volume_smooth) + 1e-10)
            power_coeffs = np.polyfit(log_v, log_p, 1)
            power_r2 = _calculate_r_squared(log_v, log_p, power_coeffs)
        except:
            power_coeffs = [0, 0]
            power_r2 = 0
        ...
        return {
            ...
            "power_law_fit": {
                "exponent": float(power_coeffs[0]),
                "coefficient": float(np.exp(power_coeffs[1])),
                "r_squared": float(power_r2)
            },
            "pv_correlation": float(np.corrcoef(pressure, volume)[0, 1])
        }
        
    except Exception as e:
        logger.error(f"Error analyzing pressure-volume: {e}")
        return {"error": str(e)}


def _compute_solvation_with_mda(traj_file: Path, solute_selection: str) -> Dict[str, Any]:
    """Compute solvation properties using MDAnalysis."""
    try:
        # This is a placeholder for MDAnalysis-based solvation analysis
        return {
            "solvation_free_energy_kj_mol": 0.0,
            "coordination_number": 0.0,
            "method": "mdanalysis_placeholder"
        }
    except Exception as e:
        logger.error(f"Error in MDAnalysis solvation calculation: {e}")
        return {"error": str(e)}


def _compute_solvation_simple(sim: Any) -> Dict[str, Any]:
    """Simple solvation property calculation."""
    try:
        # Placeholder for simple solvation calculation
        return {
            "solvation_free_energy_kj_mol": 0.0,
            "method": "simple_placeholder"
        }
    except Exception as e:
        logger.error(f"Error in simple solvation calculation: {e}")
        return {"error": str(e)}
